...
class T5Attention(mindspore.nn.Cell):
    def __init__(self, model_dim, inner_dim, num_heads, relative_attention_bias, dtype, operations):
        ...
        if relative_attention_bias:
            ...
            self.relative_attention_bias = operations.Embedding(self.relative_attention_num_buckets, self.num_heads, dtype=dtype)
...
class T5Block(mindspore.nn.Cell):
    def __init__(self, model_dim, inner_dim, ff_dim, ff_activation, gated_act, num_heads, relative_attention_bias, dtype, operations):
        super().__init__()
        self.layer = mindspore.nn.CellList()
        self.layer.append(T5LayerSelfAttention(model_dim, inner_dim, ff_dim, num_heads, relative_attention_bias, dtype, operations))
...
class T5Stack(mindspore.nn.Cell):
    def __init__(self, num_layers, model_dim, inner_dim, ff_dim, ff_activation, gated_act, num_heads, relative_attention, dtype, operations):
        super().__init__()

        self.block = mindspore.nn.CellList(
            [T5Block(model_dim, inner_dim, ff_dim, ff_activation, gated_act, num_heads, relative_attention_bias=((not relative_attention) or (i == 0)), dtype=dtype, operations=operations) for i in range(num_layers)]
        )
        self.final_layer_norm = T5LayerNorm(model_dim, dtype=dtype, operations=operations)
        # self.dropout = nn.Dropout(config.dropout_rate)

    def construct(self, x, attention_mask=None, intermediate_output=None, final_layer_norm_intermediate=True, dtype=None, embeds_info=[]):
        mask = None
...
class T5(mindspore.nn.Cell):
    def __init__(self, config_dict, dtype, operations):
        ...
        self.encoder = T5Stack(self.num_layers, model_dim, inner_dim, config_dict["d_ff"], config_dict["dense_act_fn"], config_dict["is_gated_act"], config_dict["num_heads"], config_dict["model_type"] != "umt5", dtype, operations)
        self.dtype = dtype
